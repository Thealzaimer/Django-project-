...
2. **The Bridge (MCP):** The LLM transmits this payload to our system using the Model Context Protocol. It hits our secure HTTP API bridge (`/api/mcp/call_tool/`).
...
| **c) Practical part:** Prototype one MCP-compatible interface backed by Django services. | We purposefully built the HTTP Web API Bridge (`core/views.py`), turning Django into a fully structured online MCP interface. |
